...
public abstract class AbstractItemLayout extends AbstractSelect implements HasComponents
{
	...
	public void changeVariables(final Object pSource, final Map<String, Object> pVariables)
	{
		...
		{
			String key = (String) pVariables.get(ItemLayoutConstant.ATTRIBUTE_ITEM_CLICKED_KEY);
	...
	private void paintItems(final PaintTarget target) throws PaintException
	{
		...
		for (Entry<Object, Component> entry : itemsComponent.entrySet())
		{
	...
	protected void updateItemComponents()
	{
		...
		// Keep old reference
		Set<Property<?>> oldListenedProperties = listenedProperties;
		Set<Component> oldVisibleComponents = new HashSet<Component>(itemsComponent.values());
	    ...
	    final Set<Component> pOldVisibleComponents)
	{
		...
		{
			Set<Component> visibleComponents = new HashSet<Component>(itemsComponent.values());
			for (Component c : pOldVisibleComponents)
			{
		...
		{
			for (Property<?> property : pOldListenedProperties)
			{
				Property.ValueChangeNotifier o = (ValueChangeNotifier) property;
	...
	private void handleClickEvent(final Object pItemId)
	{
		...
		{
			boolean isAlreadySelected = isSelected(pItemId);
